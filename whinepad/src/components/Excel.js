...
function reducer(data, action) {
    if (action.type === 'sort') {
        ...
        return clone(data).sort((a, b) => {
            ...
            return descending ? a[column] < b[column]
                ? 1 : -1 : a[column] > b[column]
                    ? 1 : -1;
        });
    }
    if (action.type === 'save') {
        const {int, edit} = action.payload;
        data[edit.row][edit.column] = int ? parseInt(action.payload.value, 10) : action.payload.value;
        return data;
    }
    ...
    if (action.type === 'saveForm') {
        Array.from(action.payload.form.current).forEact((input) => (data[action.payload.rowidx][input.id] = input.value),);
    }
                ...
                <thead onClick={sort}>
                    <tr>
                        {Object.keys(schema).map((key) => {
                            ...
                            return (
                                <th key={key} data-id={key}>{label}</th>
                            );
                        })}
                                    ...
                                    return (
                                        <td key={columnidx}
                                            ...
                                            className={classNames({
                                                ...
                                                ExcelDataLeft: config.aligh === 'left',
